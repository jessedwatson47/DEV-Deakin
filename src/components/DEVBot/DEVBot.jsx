    ...
    <div className="flex flex-col w-fit fixed right-2 bottom-2 z-20 items-end">
      ...
      {!isOpen ? (
        <div className="mx-auto flex ring-1 ring-black rounded w-fit gap-4 justify-between bg-black px-4 py-2">
          ...
          <button onClick={() => setIsOpen(true)} className="text-white cursor-pointer hover:text-teal-500">
            <PlusIcon />
          </button>
        </div>
      ) : (
        <>
          <div className="flex ring-1 ring-black rounded-t w-[20vw] justify-between bg-black px-4 py-2">
            ...
            <button onClick={() => setIsOpen(false)} className="text-white cursor-pointer hover:text-teal-500">
              <Cross1Icon />
            </button>
          </div>
        ...
        <MarkDown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeHighlight]}>
          {response}
            ...
            <span className="text-zinc-300">Sorry, I'm currently offline!</span>
          </div>
          <span className="bg-white text-zinc-800 p-4 h-10 w-full ring-1 ring-zinc-200 placeholder:text-zinc-500">DEVBot is currently disabled.</span>
          <button disabled={true} className="cursor-pointer bg-black text-white rounded-b w-full h-10 ring-1 ring-black disabled:opacity-50 disabled:cursor-not-allowed">
            Send
          </button>
        </>
      )}
